function varargout=matlab_figures(what, varargin)
...
switch (what)
    ...
    case 'training_performance' % Fig. 2C & 2E
    ...
    case 'trial_by_trial_corr_ET_MD'
        % Find the the trial by trial correlation of ET and MD 
    ...
    case 'measure_reliability'
        % leave-one-out reliability of mean deviation
        data = dload(fullfile(project_path,'analysis','efc1_chord.tsv'));
    ...
    case 'training_finger_count'
        % rm-ANOVA for effect of finger count in MD and ET
        C = dload(fullfile(project_path,'analysis','training_performance.tsv'));
    ...
    case 'training_repetition'
        % repetition effect analysis (interesting but not in the paper)
    ...
    case 'avg_chord_EMG_heatamp' % Fig. 4A
        
        % Just average heatmaps for chord EMGs 
    ...
    case 'model_comparison_predicting_MD' % Fig. 4B
        
        % Comparing muscle vs force vs visual complexity models in
        % predicting mean deviation of chords
        measure = 'MD';
    ...
    case 'magnitude_vs_MD_correlation' % Fig. 5B
        
        % correlation of magnitude measure with mean deviation
    ...
    case 'magnitude_and_natural_statistics_models' % Fig. 5C
    ...
    case 'natural_statistics_vs_MD_correlation' % Fig. 5D
        
        % correlation of naturality measure with mean deviation
        C = dload(fullfile(project_path,'analysis','natChord_analysis.tsv'));
        data = dload(fullfile(project_path,'analysis','efc1_chord.tsv'));
        chords_emg = C.chordID(C.sn==1 & C.sess==1);
        data = getrow(data,ismember(data.chordID,chords_emg));
        ...
        fprintf('corr: %.6f, %.6f, %.6f\n',model_corr(1),model_corr(2),model_corr(3))
        fprintf('corr significance: %.6f, %.6f, %.6f\n',p_val(1),p_val(2),p_val(3))
    case 'explained_variance_by_natural_PCs' % Fig 6
        
        % how much variance of Chord EMG and Natural EMG is captured
        % by natural EMG principal components
    ...
    case 'visual_complexity_hypothesis_testing'
        % Testing whether the complexity of the visual cue can be related 
        % to mean deviation or not.
